...
WtEnterLd, EtSpots, EntryPass, LdSpots, WtEnterEt, DmpdSoil, truckCap):
    ...
    while (DmpdSoil.level + soil_ready_to_dump) > (SoilAmt - (truckCap*50)): #wait here if SoilAmt met
        yield env.timeout(1)
    ...
    soil_ready_to_dump += (truckCap*50)
...
WtEnterLd, EtSpots, EntryPass, LdSpots, WtEnterEt, DmpdSoil, truckCap):
    ...
    yield DmpdSoil.put(truckCap*50)
    soil_ready_to_dump -= (truckCap*50)
...
def monitor(env, DmpdSoil, TrkWtLd, SoilAmt, nTrucks, truckCap):
    ...
    while True:
        ...
        if (DmpdSoil.level > (SoilAmt-(truckCap*50))):
            ...
            ProdRate = %.4f \n
            UnitCst = %.4f
            """ % (nTrucks, truckCap*50, num_of_load, num_of_dump, num_of_return, soil_ready_to_dump,
...
def main():
    ...
    print "Minimum unit cost = %.4f at optTrucks = %d and optCap = %d. Verify Val = %.4f" % (
    minUnitCst, optTrucks, optCap*5, c[optTrucks][optCap])

    #plot
    fig = plt.figure()
    ax = fig.gca(projection='3d')
    truckCapCopy = np.copy(truckCapacities_array)
    truckCapCopy *= 50
    X, Y = np.meshgrid(truckCapCopy, nTrucks_array)
    Z = UnitCst
...
def run_sim(truckCap, nTrucks):
    ...
    SlInTrk = simpy.Container(env, init=0, capacity=(truckCap*50)+3)
