...
# function to multiply the Hessian of `ys` wrt `xs` by `v`
# def _hessian_vector_product(ys, xs, v):
#   # Validate the input
#   length = len(xs)
#   # print("-----tf.shape(ys) = ", tf.shape(ys))
#   # print("-----tf.shape(xs) = ", tf.shape(xs))
#   # print("-----tf.shape(v) = ", tf.shape(v))
#   # if len(v) != length:
...
#print("!!!!! len(theta_w) = ", len(theta_w))
#print("!!!!! len(theta_b) = ", len(theta_b))
...
def line_search(x, g, delta, ob, action, advntg, old_action_probs, max_attempts=10):
    if isinstance(x, (int, long)) or isinstance(g, (int, long)) or x.dot(g) <= 0:
        return False
...
def main():
    ...
    plotting.plot_episode_stats(stats, name="Manual2_TRPO", smoothing_window=20)
