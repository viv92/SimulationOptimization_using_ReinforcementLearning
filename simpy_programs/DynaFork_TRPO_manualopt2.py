...
# #utility function to get tensor values
# def get_var(name):
...
#
# #utility function
# def replace_none_with_zero(t, shape):
...
#define theta variable (weights of neural network)
# policy_hidden_weight_matrix1, policy_hidden_weight_matrix2 = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, 'policy_hidden')
# policy_hidden_weight_vector1 = tf.reshape(policy_hidden_weight_matrix1, [-1]) #vectorize
# policy_hidden_weight_vector2 = tf.reshape(policy_hidden_weight_matrix2, [-1]) #vectorize
# policy_hidden_weight_vector = tf.concat([policy_hidden_weight_vector1, policy_hidden_weight_vector1], 0)
# policy_output_weight_matrix1, policy_output_weight_matrix2 = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, 'policy_output')
# policy_output_weight_vector1 = tf.reshape(policy_output_weight_matrix1, [-1]) #vectorize
# policy_output_weight_vector2 = tf.reshape(policy_output_weight_matrix2, [-1]) #vectorize
# policy_output_weight_vector = tf.concat([policy_output_weight_vector1, policy_output_weight_vector2], 0)
# theta = tf.concat([policy_hidden_weight_vector, policy_output_weight_vector], 0)
...
kl_hessian_vector_prod = tf.gradients(kl_grad_vector_prod, theta)
...
def conjugate_gradient(ob, action, advntg, old_action_probs, cg_iters=10, tolerance=1e-10):
    ...
    b = g.eval(feed_dict={obs: ob, chosen_action: action, advantage: advntg})
    ...
    for i in range(cg_iters):
        z = kl_hessian_vector_prod.eval(feed_dict={obs: ob, action_probs_old: old_action_probs, chosen_action: action, advantage: advntg, v: d})
...
def line_search(x, g, delta, ob, action, advntg, old_action_probs, max_attempts=10):
    ...
    theta = theta.eval()
    #begin line search
    for shrink_factor in (0.8 ** np.arange(max_backtracks)):
        #new step size
        beta *= shrink_factor
        #apply new weights
        new_theta = theta + beta * x
        sess.run(tf.assign(theta, new_theta))
