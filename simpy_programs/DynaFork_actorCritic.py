...
critic_output = tf.contrib.layers.fully_connected(inputs=fc_shared, num_outputs=1, activation_fn=None, weights_initializer=tf.contrib.layers.xavier_initializer())
#state value
...
#optimizer
critic_optimizer = tf.train.AdamOptimizer(learning_rate=alpha_critic)
#training op
critic_train_op = critic_optimizer.minimize(critic_loss)

#actor network
actor_hidden = tf.contrib.layers.fully_connected(inputs=fc_shared, num_outputs=12, activation_fn=tf.nn.relu, weights_initializer=tf.contrib.layers.xavier_initializer())
actor_output = tf.contrib.layers.fully_connected(inputs=actor_hidden, num_outputs=nA, activation_fn=None, weights_initializer=tf.contrib.layers.xavier_initializer())
...
#optimizer
actor_optimizer = tf.train.AdamOptimizer(learning_rate=alpha_actor)
#training op
